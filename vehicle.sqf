...
_delay = if (count _this > 1) then {_this select 1} else {WS_CFG_vehDelay};
_deserted = if (count _this > 2) then {_this select 2} else {WS_CFG_vehDesertDelay};
...
{	
	...
	{	
		...
		if (_explode) then {_effect = "M_Air_AT" createVehicle getPosASL _unit; _effect setPosASL getPosASL _unit;};
		sleep 0.1;
